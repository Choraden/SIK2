    ...
    if (argc > 1) {
        ...
        for (;;) {
            switch (getopt(argc, argv, "n:p:i:r:")) {
                ...
                case -1:
                    if (argc > optind + 1) { /// Extra arguments that does not match any case.
...
void Client::test_parameters() {
    ...
    std::cout << "gui_server " << gui_server << std::endl;
...
void Client::process_new_game_event(uint8_t *buff, size_t &offset, uint32_t len) {
    ...
    if (offset - poz != len) { /// Validate if received length is correct.
...
void Client::process_pixel_event(uint8_t *buff, size_t &offset, uint32_t len) {
    ...
    if (player_num >= player_name.size()
        || x >= width
        || y >= height
        || offset - poz != len) { /// Validate if received length is correct and values are in bounds.
...
void Client::process_player_eliminated_event(uint8_t *buff, size_t &offset, uint32_t len) {
    ...
    if (player >= player_name.size() || offset - poz != len) { /// Validate if the length is correct and there is player of such index.
